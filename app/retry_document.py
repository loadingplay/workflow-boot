...
class PostDocumentsNotFound():
    ...
    def get_documents(self):
        ...
        workflow_id = self.__data["workflow_id"]
        date_min = self.__data["date_min"]
        date_max = self.__data["date_max"]
        order_failled = {}

        generate_orders_in_proccess = False
        ...
        for order in orders_without_invoice:
            ...
            if not order_collected["url_document"] and not extra_colection.get("bsale_nota_venta", ""):
                count = count + 1
                order_failled[order["id"]] = order["extra_info"].get("document_generation_error", "")
